...
| 12 | PERF-01 F7 | The server flushes on every newline-terminated string, so a 300 k-file checkout does ~300 k tiny `write()` calls. Switch to a byte-threshold flush. **Requires a flush-before-read audit** or the session deadlocks. | ~30 | medium | implemented — see PERF-03 mechanism (c) for the flush-before-read reasoning; the pserver auth replies, sent before `server()` sets up the drains, still flush per line |
