...
static void RCS_extract_rev_data(RCSNode *rcs, RCSVers *rev, RevStringList &lst, bool packed)
{
  char *branchversion;
  char *cpversion;
  char *key;
...
static bool should_try_convert(RCSNode *rcs, bool &packed)
{
  packed = true;
  if (!rcs->head)
    return true;//no head, only branch?
  RCSVers *vers;
  kflag expand;
  ...
  packed = (expand.flags & KFLAG_COMPRESS_DELTA) ? true : false;
...
static void RCS_convert_to_new_binary(RCSNode *rcs)
{
  ...
  for (int i = 0; i < lst.count; i ++)
    if (lst.data[i])
      RCS_extract_rev_data(rcs, lst.data[i], lst, packed);
  ...
  for (int i = 0; i < lst.count; i ++)
  {
    ...
    sprintf(delta, "%s%.*s",
      ...
      (int)lst.data[i]->text->len, lst.data[i]->text->text);
  ...
  char sbuf[256];
  snprintf(sbuf, sizeof(sbuf), ":%s:-%lld Kb\n", packed?"Bz":"B",(long long)(old_sz-new_sz)/1024);
  sbuf[255] = 0;
  cvs_output (sbuf, 0);
}
...
int main(int ac,  const char *av[])
{
  if (ac < 3)
  {
    printf("usage: cvs_root full_file_path,v\n");
    return 1;
  }
  static char *localhost="127.0.0.1";
  caddressed_fs::set_root(caddressed_fs::get_default_ctx(), av[1]);
  #if !_WIN32
  lock_server=localhost;
  #endif
