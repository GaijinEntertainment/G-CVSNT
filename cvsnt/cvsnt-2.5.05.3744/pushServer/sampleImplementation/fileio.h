...
extern size_t blob_get_file_size(const char*);//to get blob file size for SIZE request
extern bool blob_is_file_readable(const char*);//to check if blob already exist (CHCK, PULL and PUSH requests)
...
extern bool blob_ensure_dir(const char* name);
...
extern FILE* blob_get_temp_file(std::string &fn, const char *tmp_path=nullptr, const char *mode="wb");
