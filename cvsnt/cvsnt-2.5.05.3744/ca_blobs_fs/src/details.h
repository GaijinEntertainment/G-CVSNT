...
namespace caddressed_fs
{
  ...
  int64_t repack(const context *, const char* hash_hex_string);//only exposed for tools. This call ensures that blob is compressed with best possible compression. returns size of difference (CAN be negative is zlib changed to zstd. zstd is faster)
}