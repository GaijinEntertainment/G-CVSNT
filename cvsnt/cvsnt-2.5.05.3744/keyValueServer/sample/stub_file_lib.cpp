...
uintptr_t blob_start_push_data(const void *, const char* htype, const char* hhex, uint64_t size){
  std::lock_guard<std::mutex> lck (mtx);
  auto it = storage.find(std::string(hhex));
  if (it != storage.end())
...
uintptr_t blob_start_pull_data(const void *, const char* htype, const char* hhex, uint64_t &sz)
{
  std::lock_guard<std::mutex> lck (mtx);
  auto it = storage.find(std::string(hhex));
