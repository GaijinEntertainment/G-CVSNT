...
#define binary_file_p(buf, size) (encoding.encoding || memchr (buf, '\0', size) != 0)

extern diff_encoding_type encoding; /* Override detection if --encoding passed */
...
#ifdef HAVE_SETMODE
	  if(!binary_input && encoding.encoding)
	  {
			size_t n;
			/* Check for unicode, switch to binary I/O if true */
			setmode (current->desc, O_BINARY);
			n = read (current->desc, current->buffer, max(current->bufsize,256));
			if (n == -1)
				pfatal_with_name (current->name);
			if (lseek (current->desc, - (off_t) n, SEEK_CUR) == -1)
				pfatal_with_name (current->name);
  ...
  if (current->desc < 0)
      ...
      if (current->buffered_chars < current->stat.st_size)
	{
	  ...
	  if(!binary_input && encoding.encoding)
	  {
			...
			begin_encoding(&encoding);
			if(convert_encoding(current->buffer,current->buffered_chars,&newbuf, &newsize))
			{
				free(current->buffer);
				current->buffer = newbuf;
				current->buffered_chars = newsize;
				current->bufsize = newsize;
			}
			end_encoding();
			strip_crlf(current->buffer,&current->buffered_chars);
	  }
	}
    }
  ...
  while ((char const *) p < suffix_begin)
   ...
   hashing_done:;
      ...
      if (line == alloc_lines)
	{
	  ...
	  linbuf = (char const **) realloc ((void*)(linbuf + linbuf_base),
	 ...
	 int binary;
{
  ...
  if (! (no_diff_means_no_output
      ...
      while (p0 != end0)
	{
	  ...
	  if (l == alloc_lines0)
	    linbuf0 = (char const **) realloc ((void*)linbuf0, (alloc_lines0 *= 2)
							 * sizeof(*linbuf0));
	  linbuf0[l] = p0;
	  while (*p0++ != '\n')
	    ;
