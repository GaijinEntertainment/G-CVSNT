...
static void mydbm_load_file(FILE *, List *);
...
int mydbm_store (DBM *db, datum key, datum value, int flags)
{
    ...
    node->key = (char*)xmalloc (key.dsize + 1);
    strncpy (node->key, key.dptr, key.dsize);
    node->key[key.dsize] = '\0';

    node->data = (char*)xmalloc (value.dsize + 1);
...
static void mydbm_load_file (FILE *fp, List *list)
{
    char *line = NULL;
    size_t line_size;
    char *value;
    ...
    value_allocated = 1;
    value = (char*)xmalloc (value_allocated);
