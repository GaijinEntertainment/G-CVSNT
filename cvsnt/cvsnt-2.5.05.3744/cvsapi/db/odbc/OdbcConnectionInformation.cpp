...
const char *COdbcConnectionInformation::getVariable(const char *name)
{
...
bool COdbcConnectionInformation::setVariable(const char *name, const char *value)
{
...
const char *COdbcConnectionInformation::enumVariableNames(size_t nVar)
{
	switch(nVar)
	{
...
#ifndef _WIN32
bool COdbcConnectionInformation::connectionDialog(const void *parentWindow)
{
	return false;
}
#else
...
INT_PTR CALLBACK COdbcConnectionInformation::ConnectionDialogProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	COdbcConnectionInformation *pCI = (COdbcConnectionInformation*)GetWindowLongPtr(hWnd,GWLP_USERDATA);
	switch(uMsg)
	{
		case WM_INITDIALOG:
			pCI = (COdbcConnectionInformation*)lParam;
